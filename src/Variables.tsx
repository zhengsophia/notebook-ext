...
const getColorForFrequency = (frequency: number, minFreq: number, maxFreq: number) => {
...
const CustomTreeItem = React.forwardRef(function CustomTreeItem(
  ...
  const item = publicAPI.getItem(props.itemId) as TreeViewBaseItem & { frequency?: number };

  const backgroundColor = item.frequency !== undefined
...
export default function VariableTreeView({ data }: { data: any }) {
  const allFrequencies = data.flatMap((d: any) => d.variables.map((v: any) => v.frequency));
...
// LIST VERSION
// export default function List({ data }: { data: { cluster: string; variables: { name: string; frequency: number }[] }[] }) {
...
//   const allFrequencies = data.flatMap(({ variables }) => variables.map(v => v.frequency));
//   const minFreq = Math.min(...allFrequencies);
//   const maxFreq = Math.max(...allFrequencies);
...
//                   variables.reduce((acc, v) => acc + v.frequency, 0), minFreq, maxFreq
//                 ),
//                 color: getTextColor(getColorForFrequency(
//                   variables.reduce((acc, v) => acc + v.frequency, 0), minFreq, maxFreq
...
//                   style={{
//                     backgroundColor: getColorForFrequency(frequency, minFreq, maxFreq), // Apply dynamic color
//                     color: getTextColor(getColorForFrequency(frequency, minFreq, maxFreq)), // Dynamically change text color
//                   }}
//                 >
//                   {name}
//                 </span>
